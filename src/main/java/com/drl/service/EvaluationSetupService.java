...
public class EvaluationSetupService {
    ...
    public void createEvaluationSetups(EmployeeEvaluationSetupDTO dto) {
            ...
            // Validate campaign       
            Optional<HrEvaluationCampaign> campaign = evaluationCampaignRepository.findFirstByName(dto.getEvaluationCampaign());
            if (campaign.isEmpty()) {
                try {
            ...
            for (HrEmployee employee : dto.getEmplist()) {
                ...
                if (setup != null) {
                    throw new IllegalStateException("Evaluation Setup of Employee " + employee.getName() + " already created");
                }
                ...
                if (dto.getEvaluatorlist() != null && !dto.getEvaluatorlist().isEmpty()) {
                    for (EvaluatorDTO evaluatorDTO : dto.getEvaluatorlist()) {
                        User evaluator = userRepoistory.findByTxtUserName(evaluatorDTO.getUserName())
                                .orElseThrow(() -> new IllegalArgumentException("Evaluator not found: " + evaluatorDTO.getUserName()));
    ...
    public EmployeeEvaluationSetupDTO updateEvaluationSetup(EmployeeEvaluationSetupDTO dto) {
        ...
        try {
            ...
            if (!hasFactors && !hasEvaluators) {
                throw new IllegalArgumentException("Nothing to update! Please provide either a 'factorlist' or 'evaluatorlist' in your request body.");
            }

            if (dto.getFactorlist() == null) System.out.println("Warning: factorlist is NULL in DTO");
            if (dto.getEvaluatorlist() == null) System.out.println("Warning: evaluatorlist is NULL in DTO");
            ...
            for (HrEmployee employeeDto : dto.getEmplist()) {
                ...
                if (employeeDto.getEmpId() != null) {
                    ...
                    if (emp != null) System.out.println("Found employee by empId: " + emp.getName());
                } else if (employeeDto.getEmployeeId() != null) {
                    emp = employeeRepository.findFirstByEmployeeId(employeeDto.getEmployeeId());
                    if (emp != null) System.out.println("Found employee by employeeId: " + emp.getName());
                }
                ...
                System.out.println("Looking for setups with Campaign: " + campaign.get().getName() + " (ID: " + campaign.get().getCampaignId() + ") and Employee: " + emp.getName() + " (ID: " + emp.getEmpId() + ")");
                List<EmpEvaluationSetup> setups = evaluationSetupRepository.findAllByCampaignAndEmployee(campaign.get(), emp);
                ...
                for (EmpEvaluationSetup setup : setups) {
                    ...
                    if (!canUpdate) {
                        throw new IllegalStateException("Cannot update the evaluation because evaluation has already been started");
                    }

                    // Update the factors 
                    if (dto.getFactorlist() != null) {
                        System.out.println("Refreshing factors for Setup ID: " + setup.getId() + ". New count: " + dto.getFactorlist().size());
                    ...
                    if (dto.getEvaluatorlist() != null) {
                        System.out.println("Refreshing evaluators for Setup ID: " + setup.getId() + ". New count: " + dto.getEvaluatorlist().size());
    ...
    public ResponseEntity<Object> deleteEmployeeSrtup(Integer employeeId, Integer campaignId) {
        try {
            ...
            // Find setup record
            List<EmpEvaluationSetup> setups = evaluationSetupRepository.findAllByCampaignAndEmployee(campaign, employee);
            ...
            for (EmpEvaluationSetup setup : setups) {
                ...
                if (canDelete) {
                    ...
                    System.out.println("Cannot delete setup ID " + setup.getId() + ". It is being used in evaluations.");
                }
            }
