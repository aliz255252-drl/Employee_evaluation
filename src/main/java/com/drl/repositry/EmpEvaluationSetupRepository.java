...
public interface EmpEvaluationSetupRepository extends JpaRepository<EmpEvaluationSetup, Integer> {
	...
	List<HrEvaluationFactor> getFactorsByEmpId(@Param(value = "employeeId") Integer empId, @Param(value = "campaignId") Integer campaignId);
	...
	@Query(value = "select distinct u.txt_user_name as userName, e.evaluator_level, e.bln_evaluate from Sysmgr_tbl_users u join hr_tbl_evaluation_setup_evaluators e "
			...
			+ " ser_evaluation_setup_id in (select ser_evaluation_setup_id from hr_tbl_evaluation_setup "
			+ " where ser_employee_id = :employeeId and ser_evaluation_campaign_id = :campaignId)", nativeQuery = true)
	List<EvaluatorDTO> getEvaluatorsByEmpId(@Param(value = "employeeId") Integer empId, @Param(value = "campaignId") Integer campaignId);
    ...
    default Optional<EmpEvaluationSetup> findFirstByCampaign_CampaignIdAndEmployee_EmpId(Integer campaignId, Integer empId) {
        List<EmpEvaluationSetup> list = findAllByCampaign_CampaignIdAndEmployee_EmpId(campaignId, empId);
        return list.isEmpty() ? Optional.empty() : Optional.of(list.get(0));
    }

    default Optional<EmpEvaluationSetup> findFirstByCampaignAndEmployee(HrEvaluationCampaign campaign, HrEmployee employee) {
    ...
    @Query(value = "SELECT CASE "
    		...
    		+ "JOIN hr_tbl_evaluation_setup s ON s.ser_evaluation_setup_id = e.ser_evaluation_setup_id "
    		+ "WHERE e.ser_evaluation_setup_id = :setupId "
    		+ "AND e.bln_evaluate != 'true' "
